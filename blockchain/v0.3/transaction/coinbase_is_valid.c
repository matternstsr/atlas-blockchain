	...
	const transaction_t *coinbase, uint32_t block_index)
{
	...
	/* calc the hash of the coinbase trans and compare it with the stored ID */
	...
	if (llist_size(coinbase->inputs) != 1 || llist_size(coinbase->outputs) != 1)
		return (0);
	/* Retrieve the input and verify the block hash and trans ID */
	txi = llist_get_tail(coinbase->inputs);

	if (memcmp(txi->tx_out_hash, &block_index, sizeof(uint32_t)))
		return (0);
	/* Verify that the block hash and trans ID are zeroed out */
	for (int i = 0; i < 32; i++)
	{
		if (txi->block_hash[i] != 0 || txi->tx_id[i] != 0)
			return (0);
	}
	/* Check that the sig is zeroed out (as it should be in a coinbase trans) */
	for (int i = 0; i < 72; i++)
	{
		if (txi->sig.sig[i] != 0)
			return (0);
